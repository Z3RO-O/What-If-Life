...
Deno.serve(async (req: Request) => {
  ...
  try {
    ...
    if (!decision_id) {
      return new Response(JSON.stringify({ error: 'Decision ID is required' }), {
        status: 400,
...
async function generateSimulation(decision: DecisionData, userPatterns: any) {
  ...
  const weights =
    categoryWeights[decision.category as keyof typeof categoryWeights] || categoryWeights.career;
    ...
    career: [
      {
        ...
        description: isAlternate
          ? 'Built connections in a different industry sector'
          : 'Strengthened existing professional relationships',
        category: 'career',
      },
      {
        title: isAlternate ? 'Alternative Skill Acquisition' : 'Skill Enhancement',
        description: isAlternate
          ? 'Developed different technical competencies'
          : 'Advanced current skill set',
        category: 'education',
      },
      {
        title: isAlternate ? 'Different Leadership Opportunities' : 'Leadership Development',
        description: isAlternate
          ? 'Took on leadership roles in new context'
          : 'Advanced in current leadership track',
        category: 'career',
      },
      {
        title: isAlternate ? 'Alternative Financial Growth' : 'Financial Advancement',
        description: isAlternate
          ? 'Achieved different income trajectory'
          : 'Met current financial goals',
        category: 'finance',
      },
      {
        title: isAlternate ? 'Work-Life Balance Shift' : 'Work-Life Integration',
        description: isAlternate
          ? 'Experienced different lifestyle balance'
          : 'Maintained current lifestyle approach',
        category: 'health',
      },
    ],
    education: [
      {
        ...
        description: isAlternate
          ? 'Pursued different field of study'
          : 'Deepened current academic focus',
        category: 'education',
      },
      {
        title: isAlternate ? 'Different Peer Network' : 'Academic Community',
        description: isAlternate
          ? 'Connected with peers in alternative field'
          : 'Built relationships in current field',
        category: 'relationship',
      },
      {
        title: isAlternate ? 'Alternative Career Preparation' : 'Career Readiness',
        description: isAlternate
          ? 'Prepared for different career path'
          : 'Built foundation for current career',
        category: 'career',
      },
    ],
...
function generateInsights(
  ...
  insights.push(
    `Your ${decision.category} decisions typically show a ${userPatterns.risk_tolerance > 0.5 ? 'higher' : 'lower'} risk tolerance pattern.`
  );
  ...
  if (alternatePositive > originalPositive) {
    insights.push(
      `The alternative path shows ${alternatePositive - originalPositive} more positive outcomes in our simulation.`
    );
  } else if (originalPositive > alternatePositive) {
    insights.push(
      `Your chosen path demonstrates ${originalPositive - alternatePositive} more positive outcomes than the alternative.`
    );
  } else {
  ...
  if (decision.category === 'career') {
    insights.push(
      'Career decisions at this life stage typically have long-term compounding effects on professional growth.'
    );
  } else if (decision.category === 'education') {
    insights.push(
      'Educational choices create foundational knowledge that influences future decision-making patterns.'
    );
  }
  ...
  if (decision.importance >= 4) {
    insights.push(
      'High-importance decisions like this one tend to create significant ripple effects across multiple life domains.'
    );
  }

  // Confidence insight
  insights.push(
    'Our AI model shows high confidence in these projections based on similar decision patterns in our dataset.'
  );
...
function calculateRiskTolerance(decision: DecisionData): number {
  ...
  const text =
    `${decision.chosen_path} ${decision.alternative_path} ${decision.context}`.toLowerCase();
