      ...
      <div className="container-wide">
        <div className="text-center mb-16 lg:mb-20 content-center">
          <div className="inline-flex items-center space-x-2 accent-card rounded-full px-6 py-3 mb-8 lg:mb-12">
            ...
            <span className="text-orange-400 font-accent text-sm lg:text-base">LIMITED TIME - 40% OFF PREMIUM</span>
          </div>
          ...
          {plans.map((plan, index) => (
            <Card
              ...
              className={`relative ${plan.cardClass} ${
                plan.popular
                  ? 'scale-105 shadow-2xl white-glow'
                  : 'hover:scale-105'
            ...
            >
              ...
              <CardHeader className="text-center pb-4">
                <CardTitle className="text-xl lg:text-2xl font-accent text-white mb-2">{plan.name}</CardTitle>
                <p className="text-gray-400 mb-4 font-accent-light">{plan.description}</p>
                <div className="flex items-baseline justify-center">
                  <span className={`text-3xl lg:text-4xl font-accent font-mono ${
                    plan.popular ? 'text-orange-400' : 
                    plan.price === 'Free' ? 'text-emerald-400' : 'text-white'
                  }`}>
                    {plan.price}
                  </span>
                  <span className="text-gray-400 ml-1 font-mono text-sm lg:text-base">{plan.period}</span>
                </div>
              </CardHeader>
                  ...
                  {plan.features.map((feature, featureIndex) => (
                    <li key={featureIndex} className="flex items-start space-x-3">
                      <Check className={`h-5 w-5 mt-0.5 ${
                        plan.popular ? 'text-orange-400' : 
                        plan.price === 'Free' ? 'text-emerald-400' : 'text-blue-400'
                      } flex-shrink-0`} />
        ...
        <div className="text-center mt-12 lg:mt-16">
          <div className="inline-flex items-center space-x-2 pro-card rounded-full px-6 py-3">
            ...
            <span className="text-emerald-400 font-accent-light text-sm lg:text-base">30-DAY MONEY-BACK GUARANTEE</span>
          </div>
        </div>
      </div>
    </div>
